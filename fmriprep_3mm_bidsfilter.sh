...
usage(){ echo "Usage: `basename $0` -b <bids_dir> -p <participant_id> -t <task_id> -f <bids_filter_file>
b:	bids data dir
...
BIDS filter file can be used to specify T1 from single session for processing

Example: `basename $0` -b /path/to/bids_data -p sub-235 -t foodpics -f bids_filter_file.json
" 1>&2; exit 1; }
